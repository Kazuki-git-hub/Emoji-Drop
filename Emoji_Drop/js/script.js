document.addEventListener('DOMContentLoaded', () => {
    ...
    const bgmNextButton = document.getElementById('bgm-next-button');
    const bgmPrevButton = document.getElementById('bgm-prev-button');
    ...
    function playBGM(index) {
        ...
        const bgmFile = BGM_FILES[currentBGMIndex];
        bgm = new Audio(bgmFile);
        bgm.loop = true;
        bgm.currentTime = 0;
        bgm.play();
        lastPlayedBGM = bgmFile;
    }
    ...
    function startGame() {
        ...
        updateAchievements();
        // Restartボタンを表示
        restartAlwaysButton.classList.remove('hidden');
        draw();
        gameInterval = setInterval(gameLoop, dropSpeed);
        // === BGM再生 ===
        try {
            ...
            if (BGM_FILES.length === 1) {
                candidates = BGM_FILES;
            } else if (lastPlayedBGM) {
                candidates = BGM_FILES.filter(f => f !== lastPlayedBGM);
            } else {
                candidates = BGM_FILES;
            }
            const bgmFile = candidates[Math.floor(Math.random() * candidates.length)];
            currentBGMIndex = BGM_FILES.indexOf(bgmFile);
            bgm = new Audio(bgmFile);
            bgm.loop = true;
            bgm.currentTime = 0;
            bgm.play();
    ...
    emojiSwitchButton.addEventListener('click', () => {
        ...
        // 現在ドロップ中のテトリミノの絵文字だけ新セットに変換（形はそのまま）
        if (currentTetromino) {
            let idx = -1;
            for (let set of EMOJI_SETS) {
                let i = set.indexOf(currentTetromino.emoji);
                if (i !== -1) { idx = i; break; }
            }
            if (idx !== -1) {
                currentTetromino.emoji = EMOJIS[idx];
            }
        }
        // 次のテトリミノは新セットで再生成
        nextTetromino = getRandomTetromino();
    ...
    // すべてのボタンで、マウスクリック後にフォーカスを外すことでスペースキーによる再クリックを防ぐ
    ...
    // change Emojiボタンとリスタートボタンで、クリック後にフォーカスを外す
    [emojiSwitchButton, restartButton, restartAlwaysButton].forEach(btn => {
        if (btn) {
    ...
    // === BGMファイルリスト ===
    const BGM_FILES = [
        './bgm/tetris-theme-korobeiniki-rearranged-arr-for-music-box-184978.mp3',
        './bgm/tetris-theme-korobeiniki-arranged-for-piano-186249.mp3',
        './bgm/tetris-theme-korobeiniki-rearranged-arr-for-strings-185592.mp3'
    ];
    let bgm = null;
    let lastPlayedBGM = null;
    ...
    if (bgmNextButton) {
        bgmNextButton.addEventListener('click', () => {
            currentBGMIndex = (currentBGMIndex + 1) % BGM_FILES.length;
            playBGM(currentBGMIndex);
        });
    }
    if (bgmPrevButton) {
        bgmPrevButton.addEventListener('click', () => {
            currentBGMIndex = (currentBGMIndex - 1 + BGM_FILES.length) % BGM_FILES.length;
            playBGM(currentBGMIndex);
        });
    }
